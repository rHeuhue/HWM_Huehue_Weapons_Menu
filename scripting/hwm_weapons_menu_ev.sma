...
new const g_szWeapons_FileName[] = "Huehue_Weapons_List_EV"
...
new Array:g_aWeapons_Short[eSections], Array:g_aWeapons_Menu[eSections], Array:g_aWeapons_Price[eSections], Array:g_aWeapons_VipFlag[eSections],
	Array:g_aWeapons_ViewSkin[eSections], Array:g_aWeapons_PlayerSkin[eSections], Array:g_aWeapons_Team[eSections]
...
public plugin_init()
{
	register_plugin("[HWM] Huehue Weapon Menu", "1.0.7-EV Personalized", "Huehue @ AMXX-BG.INFO")
...
public plugin_precache()
{
	for (new iSection = _:SECTION_PRIMARY; iSection <= _:SECTION_BOT_SECONDARY; iSection++)
	{
		...
		g_aWeapons_Team[Sections:iSection] = ArrayCreate(ARRAY_SIZE, ARRAY_SIZE)
	}
...
public CBasePlayer_Killed(iVictim, iKiller, iShouldGib)
{
	...
	{
		rg_give_item_ex(iKiller, "weapon_hegrenade", GT_APPEND)

		if (rg_get_user_bpammo(iKiller, WEAPON_HEGRENADE) < g_eCvars[HWM_MAX_HE_NADES])
		{
			rg_set_user_bpammo(iKiller, WEAPON_HEGRENADE, rg_get_user_bpammo(iKiller, WEAPON_HEGRENADE) + 1)
		}
	}
...
public HWM_Reload_Weapons_File(id, level, cid)
{
	...
	for (new iSection = _:SECTION_PRIMARY; iSection <= _:SECTION_SECONDARY; iSection++)
	{
		...
		ArrayClear(g_aWeapons_Team[Sections:iSection])
	}
...
ShowPrimaryWeapons(id)
{
	...
	static iWeaponPrice, szWeaponMenuName[MAX_NAME_LENGTH], szTeam[10], TeamName:iTeam, szWeaponId[10]
	...
	for (new i = 0; i < ArraySize(Array:g_aWeapons_Short[SECTION_PRIMARY]); i++)
	{
		...
		ArrayGetString(Array:g_aWeapons_Team[SECTION_PRIMARY], i, szTeam, charsmax(szTeam))

		if (equal(szTeam, "CT"))
			iTeam = TEAM_CT
		else if (equal(szTeam, "T"))
			iTeam = TEAM_TERRORIST
		else if (equal(szTeam, "ANY") || equal(szTeam, ""))
			iTeam = TEAM_UNASSIGNED

		if (rg_get_user_team(id) == iTeam || iTeam == TEAM_UNASSIGNED)
		{
			num_to_str(i, szWeaponId, charsmax(szWeaponId))

			if (iWeaponPrice > 0 && !is_user_flagged(id, i, SECTION_PRIMARY))
				menu_additem(g_iMenuIDs[PRIMARY_ID], fmt("%L", id, "HWM_PRIMARY_WEAPON_PRICE_MENU", szWeaponMenuName, is_user_vip(id) ? discount_math_fix(iWeaponPrice, g_eCvars[HWM_VIP_DISCOUNT]) : iWeaponPrice), szWeaponId)
			else if (is_user_flagged(id, i, SECTION_PRIMARY) || iWeaponPrice == 0)
				menu_additem(g_iMenuIDs[PRIMARY_ID], szWeaponMenuName, szWeaponId)
		}
	}
...
public PrimaryHandler(id, iMenu, Item)
{
	...
	new szData[MAX_ITEM_TYPES]
	menu_item_getinfo(iMenu, Item, .info = szData, .infolen = charsmax(szData))

	new iWeapon = str_to_num(szData)

	static iWeaponPrice, iTotalPricePrim, iSecondaryPrice
	iWeaponPrice = ArrayGetCell(Array:g_aWeapons_Price[SECTION_PRIMARY], iWeapon)
	...
	if (!is_user_flagged(id, iWeapon, SECTION_PRIMARY))
	{
		if (g_iWeapons[id][SECONDARY] != -1)
		{
...
ShowSecondaryWeapons(id)
{
	...
	for (new i = 0; i < ArraySize(Array:g_aWeapons_Short[SECTION_SECONDARY]); i++)
	{
		...
		ArrayGetString(Array:g_aWeapons_Team[SECTION_SECONDARY], i, szTeam, charsmax(szTeam))
		...
		if (rg_get_user_team(id) == iTeam || iTeam == TEAM_UNASSIGNED)
		{
			...
			if (iWeaponPrice > 0 && !is_user_flagged(id, i, SECTION_SECONDARY))
				menu_additem(g_iMenuIDs[SECONDARY_ID], fmt("%L", id, "HWM_SECONDARY_WEAPON_PRICE_MENU", szWeaponMenuName, is_user_vip(id) ? discount_math_fix(iWeaponPrice, g_eCvars[HWM_VIP_DISCOUNT]) : iWeaponPrice), szWeaponId)
			else if (is_user_flagged(id, i, SECTION_SECONDARY) || iWeaponPrice == 0)
				menu_additem(g_iMenuIDs[SECONDARY_ID], szWeaponMenuName, szWeaponId)
		}
	}
...
public SecondaryHandler(id, iMenu, Item)
{
	...
	iWeaponPrice = ArrayGetCell(Array:g_aWeapons_Price[SECTION_SECONDARY], iWeapon)

	if (g_iWeapons[id][PRIMARY] != -1)
		iPrimaryPrice = ArrayGetCell(Array:g_aWeapons_Price[SECTION_PRIMARY], g_iWeapons[id][PRIMARY])

	if (!is_user_flagged(id, iWeapon, SECTION_SECONDARY))
	{
		if (g_iWeapons[id][PRIMARY] != -1)
		{
			if (iPrimaryPrice > 0)
	...
	g_iWeapons[id][SECONDARY] = iWeapon
...
UTIL_Load_Weapons_File(bool:bReloadFile)
{
	...
	if (!file_exists(szFileName))
	{
		...
		if (iFilePointer)
		{
			...
			formatex(szFileDetails, charsmax(szFileDetails), "; To comment line use one of those symbols: # ; // ^n\
										...
										; Team -> ^"CT^" for CTs only | ^"T^" for Ts only | ^"ANY^" for both teams to appear in the menu for each team^n\
										; Example: ^n\
										; ^"awp^" ^"AWP Magnum^" 4750 ^"d^" ^"models/ex_folder/v_awp.mdl^" ^"models/ex_folder/p_awp.mdl^" ^"ANY^" -> Players with flag 'd' will get it for free, other members like VIP will get it -% discount price setted by cvar & normal players will pay 4750 ^n^n\
										[PRIMARY]^n\
										^"m4a1^" ^"\w[\yM4A1\w]^" 0 ^"free^" ^"default^" ^"default^" ^"CT^"^n\
										^"ak47^" ^"\w[\yAK47\w]^" 0 ^"free^" ^"default^" ^"default^" ^"T^" ^n\
										^"aug^" ^"AUG^" 0 ^"free^" ^"default^" ^"default^" ^"CT^"^n\
										^"sg552^" ^"SG552^" 0 ^"free^" ^"default^" ^"default^" ^"T^"^n\
										^"galil^" ^"Galil^" 0 ^"free^" ^"default^" ^"default^" ^"T^"^n\
										^"famas^" ^"Famas^" 0 ^"free^" ^"default^" ^"default^" ^"CT^"^n\
										^"scout^" ^"Scout^" 0 ^"free^" ^"default^" ^"default^" ^"ANY^"^n\
										^"awp^" ^"AWP^" 4750 ^"d^" ^"default^" ^"default^" ^"ANY^"^n\
										^"sg550^" ^"SG550^" 4200 ^"b^" ^"default^" ^"default^" ^"ANY^"^n\
										^"m249^" ^"M249^" 5750 ^"a^" ^"default^" ^"default^" ^"ANY^"^n\
										^"g3sg1^" ^"G3SG1^" 5000 ^"c^" ^"default^" ^"default^" ^"ANY^"^n\
										^"p90^" ^"P90^" 0 ^"free^" ^"default^" ^"default^" ^"ANY^"^n^n\
										[SECONDARY]^n\
										^"glock18^" ^"Glock 18^" 0 ^"free^" ^"default^" ^"default^" ^"T^"^n\
										^"usp^" ^"USP^" 0 ^"free^" ^"default^" ^"default^" ^"CT^"^n\
										^"p228^" ^"P228^" 0 ^"free^" ^"default^" ^"default^" ^"ANY^"^n\
										^"deagle^" ^"\w[\yDeagle\w]^" 0 ^"free^" ^"default^" ^"default^" ^"ANY^"^n\
										^"fiveseven^" ^"FiveSeven^" 0 ^"free^" ^"default^" ^"default^" ^"CT^"^n\
										^"elite^" ^"Elite Dual Batteras^" 0 ^"free^" ^"default^" ^"default^" ^"T^"^n^n\
										[BOT PRIMARY]^n\
										^"ak47^"^n\
										^"m4a1^"^n\
										^"galil^"^n\
	...
	static Sections:iSection, szWeaponShort[MAX_NAME_LENGTH], szWeaponMenuName[MAX_NAME_LENGTH], szPrice[10], szVipFlag[10], szViewSkin[64], szPlayerSkin[64], szTeam[10]
		...
		switch(szData[0])
		{
			...
			default:
			{
				parse(szData, szWeaponShort, charsmax(szWeaponShort), szWeaponMenuName, charsmax(szWeaponMenuName), szPrice, charsmax(szPrice), szVipFlag, charsmax(szVipFlag),
					szViewSkin, charsmax(szViewSkin), szPlayerSkin, charsmax(szPlayerSkin), szTeam, charsmax(szTeam))
				...
				ArrayPushString(g_aWeapons_Team[iSection], szTeam)
...
stock give_player_items(id)
{
	...
	static szWeaponTeam[eWeaponType][10]
	ArrayGetString(Array:g_aWeapons_Team[SECTION_PRIMARY], p_iId, szWeaponTeam[PRIMARY], charsmax(szWeaponTeam[]))
	ArrayGetString(Array:g_aWeapons_Team[SECTION_SECONDARY], s_iId, szWeaponTeam[SECONDARY], charsmax(szWeaponTeam[]))

	if (equal(szWeaponTeam[PRIMARY], "CT") && rg_get_user_team(id) != TEAM_CT
		|| equal(szWeaponTeam[PRIMARY], "T") && rg_get_user_team(id) != TEAM_TERRORIST
		|| equal(szWeaponTeam[SECONDARY], "CT") && rg_get_user_team(id) != TEAM_CT
		|| equal(szWeaponTeam[SECONDARY], "T") && rg_get_user_team(id) != TEAM_TERRORIST)
	{
		client_print(id, print_center, "Selected weapons are for the other Team, please choose again..")
		ShowWeaponsMenu(id)
		return
	}
